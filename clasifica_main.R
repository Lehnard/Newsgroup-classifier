...
# datos:
NEW_DATASETS <- TRUE    # Nuevos DT_sample, Xtrain, Xtest, ytrain e y test 
...
LOGISTIC_CLASSIF= TRUE   # Por logistic regression-"one vs all".
#==================================================================================
...
if(NEW_DATASETS) {
        ...
        # Tipo de sampleado: "uniform" o "min_desv".
        DT_sample <- create_DT_sample(num_docs= 7500,    # numero de documentos.
                                      seed= 1234,        # semilla.
                                      type= "uniform")   # tipo sampleado.
        ...
        # Generamos dataset: matriz documentos-terminos X e vector etiquetas
        ...
        Xy <- X_y_dataset(corpus, 
                          ...
                          tfidf= TRUE,            # matriz doc-term con idf o sin.
                          with.normalize= TRUE,   # matriz tfidf normalizada? 
                          with.sparse= TRUE, 
                          sparse= 0.999)         # valor de sparse por termino. se quitan 
                                                  # terminos con sparse mayor.

        # Partimos matriz terminos documentos "X" y vector "y" en train y test sets.
        sets <- train_test(X= Xy$X,
                           y= Xy$y,
                           train.part= 0.8)     # proporcion de X,y para Xtrain,ytrain        
        
        # Preparamos las piezas para entrenar el modelo y para clasificar.
        Xtrain <- sets$train$X
        ytrain <- sets$train$y
...
if(SVM_CLASSIF) {
        ...
        svm_model <- svm(Xtrain, ytrain, 
                         ...
                         probability= TRUE, 
                         kernel= "radial",
                         cost= 1)  # default: 1. C regularization parameter.
        cat(" ok!\n")
...
if(KSVM_CLASSIF) {
        ...
        # Definimos un kernel basado en la similaridad por coseno.
